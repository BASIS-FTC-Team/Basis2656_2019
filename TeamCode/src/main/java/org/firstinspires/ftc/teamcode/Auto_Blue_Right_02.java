...
import static org.firstinspires.ftc.teamcode.Parameters.BACK_WALL_TARGET_X;
...
import static org.firstinspires.ftc.teamcode.Parameters.FRONT_WALL_TARGET_X;
import static org.firstinspires.ftc.teamcode.Parameters.FRONT_WALL_TARGET_Y;
...
@Autonomous(name="Auto_Blue_Right_02",group = "Basis2656_2019")
@Disabled

public class Auto_Blue_Right_02 extends LinearOpMode {
...
//    private final double pRx = -323.8, pRy = 997.8;
//    private final double pMx = -600, pMy = 600;
//    private final double pLx = -997.8, pLy = 323.8;

    /* For autoCase.RED_RIGHT:
       pRx = 319, red_pRy = -1036, red_pMx = 600, red_pMy = -600, red_pLx = 1036, red_pLy = -319
       */
//    private final double pRx = 319, pRy = -1036, pMx = 600, pMy = -600, pLx = 1036, pLy = -319;
//    private final double corner_Target_X = RED_CORNER_TARGET_X, corner_Target_Y = RED_CORNER_TARGET_Y;
    ...
    public void runOpMode() {
        ...
        TelemetryWrapper.setLine(logCount++, String.format("[AFTloop] %d loops past gP is: %s", ++loop_times, goldPostion.toString()));
        ...
        if (goldPostion.equals(GoldPosition.UNKNOWN) ) {
            //Log
            TelemetryWrapper.setLine(logCount++, String.format("[Gold Not Detected] goldPosition UNKNOWN, set as default to LEFT."));
            //
        ...
        switch (goldPostion) {
            case RIGHT:
                ...
                driveTrainEnc.moveForthBackEnc(500,5000);
                driveTrainEnc.moveForthBackEnc(-500,5000);
                driveTrainEnc.spinEnc(-90,5000);
                driveTrainEnc.moveForthBackEnc(DIST_BTWN_MINERALS*2,5000);
                break;
            case MIDDLE:
                driveTrainEnc.moveLeftRightEnc(-FIRST_MOVE_RIGHT,5000);
                driveTrainEnc.moveForthBackEnc(800,5000);
                driveTrainEnc.moveForthBackEnc(-800,5000);
                driveTrainEnc.spinEnc(-90,5000);
                driveTrainEnc.moveForthBackEnc(DIST_BTWN_MINERALS,5000);
                break;
            case LEFT:
                driveTrainEnc.moveLeftRightEnc( -(DIST_BTWN_MINERALS + FIRST_MOVE_RIGHT), 5000);
                driveTrainEnc.moveForthBackEnc(500,5000);
        ...
        if (robotLoc.targetIsVisible()) {
            ...
            //Log
            TelemetryWrapper.setLine(logCount++,String.format("(alpha, beta) = (%.1f, %.1f)",alpha,beta));
            //

            angleToTurn1 = beta - alpha;
            //Log
            TelemetryWrapper.setLine(logCount++,String.format("(angleToTurn1) = (%.1f)",angleToTurn1 ));
            //
...
//            }
            ...
            //Log
            TelemetryWrapper.setLine(logCount++,String.format("(actual turn) = (%.1f)", -angleToTurn1 ));
            //
//            runtimeMain.reset();
//            while (runtimeMain.milliseconds() < 15000) {
//                idle();
//            }
            ...
            driveTrainEnc.moveForthBackEnc(848.5, 5000);
